...
export const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const renderNode = (node: FileNode, depth: number = 0): React.ReactNode => {
    ...
    return (
      <div key={node.path}>
        <div
          className={`flex items-center gap-1 px-2 py-1 hover:bg-surface-raised cursor-pointer group ${
        ...
        >
          {node.type === 'folder' && (
            <button
              ...
              className="w-4 h-4 flex items-center justify-center hover:bg-surface-raised rounded text-muted-foreground"
            >
              <svg
                width="12"
                height="12"
          ...
          {isRenaming ? (
            <input
              ...
              autoFocus
              className="flex-1 px-1 py-0 border border-border rounded text-sm bg-surface-subtle text-foreground"
            />
          ) : (
            <span
              onClick={() => {
              ...
              className="flex-1 text-sm whitespace-nowrap text-foreground"
              title={node.name}
            >
              {node.name}
            </span>
            ...
            <div className="opacity-0 group-hover:opacity-100 flex gap-1">
              {node.type === 'folder' && (
                <button
                  ...
                  className="p-0.5 text-muted-foreground hover:text-foreground hover:bg-surface-raised rounded"
                  title="新規ファイル"
                >
  ...
  return (
    <div
      ref={containerRef}
      className="file-explorer h-full bg-surface focus:outline-none"
      style={{
        overflowY: 'auto',
        overflowX: 'auto',
      }}
    ...
    >
      <div className="p-2 border-b border-border bg-surface-subtle sticky top-0 z-10">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-foreground">ファイル</span>
            {onCollapse && (
              <button
                onClick={onCollapse}
                className="p-1 text-muted-foreground hover:text-foreground hover:bg-surface-raised rounded transition-colors"
                title="エクスプローラーを閉じる"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex gap-1">
            ...
            {!readOnly && (
              <button
                ...
                className="text-xs px-2 py-1 bg-primary text-primary-foreground rounded hover:bg-primary-hover"
              >
                新規ファイル
              </button>
            )}
        ...
        {showShortcuts && (
          <div className="text-xs text-muted-foreground bg-surface-subtle p-2 rounded space-y-1">
            <div><kbd className="px-1 bg-background border border-border rounded">↑↓</kbd> 移動</div>
            <div><kbd className="px-1 bg-background border border-border rounded">←→</kbd> フォルダ開閉</div>
            <div><kbd className="px-1 bg-background border border-border rounded">Enter</kbd> 選択/開閉</div>
          </div>
        )}
      </div>
      <div className="group">
