...
| 2   | `POST /chat/stream`      | SSE ストリーミングチャット ✅ **実装済み** |
| 3   | `POST /context-snapshot` | 画面コンテキストのスナップショット登録     |
| 4   | `POST /error-report`     | エラー情報の登録＋解析トリガ               |
| 5   | `GET /conversation/{id}` | 会話履歴の取得（将来拡張）                 |

### 3.2 Playground API ✅ **実装済み**

| #   | エンドポイント                 | 説明                                         |
| --- | ------------------------------ | -------------------------------------------- |
| 1   | `POST /playground/chat`        | デバッグ用チャット（設定オーバーライド可能） |
| 2   | `POST /playground/chat/stream` | デバッグ用ストリーミングチャット             |
| 3   | `GET /playground/config`       | 現在の設定取得                               |

### 3.3 RAG Document 管理 API ✅ **実装済み**

| #   | エンドポイント           | 説明                    |
| --- | ------------------------ | ----------------------- |
| 1   | `GET /documents`         | ドキュメント一覧取得    |
| 2   | `POST /documents`        | ドキュメント登録        |
| 3   | `DELETE /documents/{id}` | ドキュメント削除        |
| 4   | `POST /documents/search` | ハイブリッド検索（RAG） |
