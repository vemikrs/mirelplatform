...
> mirel Studio の段階的実装ロードマップとマイルストーン
...
- [ ] **Metadata Entities**: `StuModelHeader`, `StuField`, `StuRelation` 等のメタデータ管理テーブル実装
- [ ] **Dynamic Schema Engine**: メタデータから物理 DB (PostgreSQL) へのマッピング・DDL 発行機能
- [ ] **Runtime Data Access**: `DynamicEntityService` (JSON ベースでの動的 CRUD 処理)
- [ ] **Draft/Release Management**: Draft 保存と Release スナップショット化の基本ロジック

### 2.2 Frontend (React/Vite)
- [ ] **Studio Shell**: Studio 全体のレイアウト、サイドバー、プロジェクト切り替え
- [ ] **Modeler UI**:
    - Entity 一覧・作成
    - Field 定義エディタ（型、バリデーション設定）
    - Relation 定義
- [ ] **API Preview**: 定義したモデルに対する API テストコンソール
...
- [ ] **Form Designer**:
    - Grid Layout Editor (Drag & Drop)
    - Widget Palette (Text, Number, Date, Select...)
    - Property Editor (ラベル、必須、表示条件)
- [ ] **Preview Mode**: 編集中のフォームを即時プレビュー

### 3.2 Frontend (Runtime)
- [ ] **Dynamic Form Renderer**: Form JSON 定義を解釈して React コンポーネントを展開するエンジン
- [ ] **Data Binding**: Runtime API とフォーム入力値の双方向バインディング
- [ ] **Validation**: モデル定義およびフォーム定義に基づくクライアントサイドバリデーション
