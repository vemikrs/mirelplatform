...
export default defineConfig({
  ...
  webServer: process.env.CI ? undefined : [
    {
      // DB認証情報は環境変数で設定（デフォルト値はローカル開発用）
      command: `bash -lc "cd ../../ && SPRING_PROFILES_ACTIVE=e2e SERVER_PORT=3000 DATABASE_URL=\${DATABASE_URL:-jdbc:postgresql://localhost:5433/mirelplatform_e2e} DATABASE_USER=\${DATABASE_USER:-mirel_e2e} DATABASE_PASS=\${DATABASE_PASS:-mirel_e2e_password} REDIS_PORT=6380 SMTP_PORT=1025 ./gradlew --console=plain :backend:bootRun"`,
