...
public class AzureEmailServiceImpl implements EmailService {
    ...
    public void sendPlainTextEmail(String to, String subject, String body) {
        try {
            ...
            log.info("Azure プレーンテキストメール送信成功: to={}, messageId={}", SanitizeUtil.forLog(to), result.getId());
        } catch (Exception e) {
            log.error("Azure プレーンテキストメール送信失敗: to={}, error={}", SanitizeUtil.forLog(to),
                    SanitizeUtil.forLog(e.getMessage()), e);
    ...
    public void sendHtmlEmail(String to, String subject, String htmlBody) {
        try {
            ...
            log.info("Azure HTMLメール送信成功: to={}, messageId={}", SanitizeUtil.forLog(to), result.getId());
        } catch (Exception e) {
            log.error("Azure HTMLメール送信失敗: to={}, error={}", SanitizeUtil.forLog(to),
                    SanitizeUtil.forLog(e.getMessage()), e);
            // システムエラーの詳細は隠蔽し、一般的なメッセージのみスロー
            throw new RuntimeException("メール送信に失敗しました");
        }
