...
public class PolicyEnforcer {
    ...
    public ValidationResult validateRequest(ChatRequest request) {
        ...
        // プロンプトインジェクションチェック
        var injectionResult = promptInjectionDetector.check(content);
        if (injectionResult.isHardBlock()) {
            log.warn("プロンプトインジェクション（ハードブロック）検出: score={}, patterns={}",
                    injectionResult.getScore(), injectionResult.getDetectedPatterns());
            return ValidationResult.error("セキュリティポリシーにより、このメッセージは送信できません");
        }
    ...
    public String filterResponse(String response, String systemRole) {
        ...
        // 機密情報のマスキング (PiiMasker)
        String filtered = piiMasker.mask(response);
